...
function normalizeProvisionOptions(options) {
    ...
    const uploadTtlSeconds = options.uploadTtlSeconds ?? DEFAULT_UPLOAD_TTL_SECONDS;
    ...
    const signerName = options.signerName?.trim() || `convex-fs upload signer ${signerHostname}`;
    return {
        bunnyAccountApiKey: trimRequired(options.bunnyAccountApiKey, "bunnyAccountApiKey"),
        storageZoneName: trimRequired(options.storageZoneName, "storageZoneName"),
        storageAccessKey: trimRequired(options.storageAccessKey, "storageAccessKey"),
        region: options.region?.trim() ?? "",
        signerHostname,
        signerPath,
        uploadPath,
        signerName,
        linkedPullZoneName: options.linkedPullZoneName?.trim() ||
            `convex-fs-upload-${signerHostname.replace(/[^a-zA-Z0-9-]/g, "-")}`,
        signerAccessKey: options.signerAccessKey?.trim() || generateSignerAccessKey(),
        uploadTtlSeconds,
        maxUploadBytes,
        fetch: options.fetch ?? fetch,
    };
}
function generateSignerAccessKey() {
    const bytes = new Uint8Array(32);
    crypto.getRandomValues(bytes);
    return Array.from(bytes, (byte) => byte.toString(16).padStart(2, "0")).join("");
}
function classifyBunnyApiFailure(response, body) {
    const details = parseMaybeJson(body);
    const messageFromBody = typeof details === "object" &&
        details !== null &&
        "Message" in details &&
...
async function bunnyApi(options, path, init, okStatuses) {
...
function getLinkedPullZoneId(script) {
    const id = script.LinkedPullZones?.find((zone) => zone.Id !== undefined)?.Id;
    return typeof id === "number" ? id : null;
}
function createSignerScript(options) {
...
async function findExistingScript(options) {
    const response = await bunnyApi(options, `/compute/script?type=${BUNNY_EDGE_SCRIPT_TYPE_CDN}&page=1&perPage=1000&search=${encodeQuery(options.signerName)}&includeLinkedPullzones=true`, { method: "GET" }, [200]);
    if (!response.ok) {
        return response;
    }
    return {
        ok: true,
        script: response.value.Items?.find((script) => script.Name === options.signerName) ?? null,
    };
}
async function createOrUpdateScript(options) {
    const existing = await findExistingScript(options);
    if (!existing.ok) {
        return existing;
    }
    if (existing.script?.Id !== undefined) {
        const updated = await bunnyApi(options, `/compute/script/${existing.script.Id}`, {
            method: "POST",
            body: JSON.stringify({
    ...
    const created = await bunnyApi(options, "/compute/script", {
        method: "POST",
        body: JSON.stringify({
            Name: options.signerName,
...
async function configureScript(options, scriptId) {
    const steps = [
        async () => {
            const response = await bunnyApi(options, `/compute/script/${scriptId}/code`, {
                method: "POST",
                body: JSON.stringify({ Code: createSignerScript(options) }),
            }, [204]);
            return response.ok ? { ok: true } : response;
        },
        async () => upsertSecret(options, scriptId, "STORAGE_ACCESS_KEY", options.storageAccessKey),
        async () => upsertSecret(options, scriptId, "SIGNER_ACCESS_KEY", options.signerAccessKey),
        async () => upsertVariable(options, scriptId, "STORAGE_ZONE_NAME", options.storageZoneName),
        async () => upsertVariable(options, scriptId, "STORAGE_REGION", options.region),
        async () => upsertVariable(options, scriptId, "UPLOAD_TTL_SECONDS", String(options.uploadTtlSeconds)),
        async () => upsertVariable(options, scriptId, "MAX_UPLOAD_BYTES", String(options.maxUploadBytes)),
        async () => {
            const response = await bunnyApi(options, `/compute/script/${scriptId}/publish`, {
                method: "POST",
...
async function upsertSecret(options, scriptId, name, secret) {
    const response = await bunnyApi(options, `/compute/script/${scriptId}/secrets`, {
        method: "PUT",
        body: JSON.stringify({ Name: name, Secret: secret }),
    }, [200, 204]);
    return response.ok ? { ok: true } : response;
}
async function upsertVariable(options, scriptId, name, value) {
    const response = await bunnyApi(options, `/compute/script/${scriptId}/variables`, {
        method: "PUT",
        body: JSON.stringify({ Name: name, Required: true, DefaultValue: value }),
    }, [200, 204]);
    return response.ok ? { ok: true } : response;
}
async function ensureHostname(options, pullZoneId) {
    const response = await bunnyApi(options, `/pullzone/${pullZoneId}/addHostname`, {
        method: "POST",
 ...
 */
export async function provisionBunnyEdgeUploadSigner(options) {
    let normalized;
    try {
        normalized = normalizeProvisionOptions(options);
    }
    catch (error) {
        return failure(error instanceof Error && error.message.includes("required")
            ? "missing_config"
            : "invalid_config", error instanceof Error ? error.message : "Invalid Bunny signer config.");
    }
    ...
    if (scriptId === undefined) {
        return failure("deployment_failed", "Bunny API did not return a script ID.");
    }
    ...
    if (pullZoneId === null) {
        return failure("deployment_failed", "Bunny API did not return a linked Pull Zone for the signer script.");
    }
...
//# sourceMappingURL=bunny-edge-tools.js.map