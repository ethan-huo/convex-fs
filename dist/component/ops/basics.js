...
export const stat = query({
    ...
    handler: async (ctx, args) => {
        ...
        const blob = await ctx.db
            ...
            .unique();
        if (!blob) {
            throw new Error(`Invariant violation: blob not found for blobId "${file.blobId}" (path: "${args.path}")`);
        }
        return {
            path: file.path,
            blobId: file.blobId,
...
export const list = query({
    ...
    handler: async (ctx, args) => {
        ...
        const page = await Promise.all(result.page.map(async (file) => {
            const blob = await ctx.db
                .query("blobs")
                .withIndex("blobId", (q) => q.eq("blobId", file.blobId))
                .unique();
            if (!blob) {
                throw new Error(`Invariant violation: blob not found for blobId "${file.blobId}" (path: "${file.path}")`);
            }
            return {
                path: file.path,
                blobId: file.blobId,
...
export const restore = internalMutation({
    ...
    handler: async (ctx, args) => {
        ...
        const blob = await ctx.db
            ...
            .unique();
        if (!blob) {
            throw new Error(`Blob not found: "${args.blobId}". It may have been garbage collected.`);
        }
...
export const clearAllFiles = internalAction({
    ...
    handler: async (ctx) => {
        ...
        if (!configDoc) {
            throw new Error('No config found in config table with key "storage". ' +
        ...
        if (!config.allowClearAllFiles) {
            throw new Error("clearAllFiles is disabled. To enable it, set allowClearAllFiles: true " +
