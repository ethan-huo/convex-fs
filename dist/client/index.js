...
function createClosedByteStream() {
...
function prepareUploadStream(stream, contentLength) {
...
async function cleanupBlobAfterControlPlaneFailure(store, blobId) {
    try {
        ...
        console.error(`Failed to clean up blob "${blobId}" after control-plane failure:`, error);
    }
}
/**
...
export class ConvexFS {
     ...
     * ```
     */
    async getDownloadUrl(ctx, blobId, opts) {
     ...
     */
    async getBlobStream(ctx, blobId) {
     ...
     */
    async writeBlobStream(ctx, stream, contentType, opts) {
     ...
     */
    async writeFileStream(ctx, path, stream, contentType, opts) {
     ...
     * ```
     */
    async copy(ctx, sourcePath, destPath) {
     ...
     * ```
     */
    async move(ctx, sourcePath, destPath) {
 ...
 * ```
 */
export function registerRoutes(http, component, fs, config) {
    ...
    cors.route({
        ...
        handler: httpActionGeneric(async (ctx, req) => {
            ...
            try {
                const allowed = await config.downloadAuth(ctx, blobId, path, extraParams);
                if (!allowed) {
                    return new Response(JSON.stringify({ error: "Forbidden" }), {
                        status: 403,
                        headers: { "Content-Type": "application/json" },
 ...
 */
export function buildDownloadUrl(siteUrl, prefix, blobId, path, params) {
