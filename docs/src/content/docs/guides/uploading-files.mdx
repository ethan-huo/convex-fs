...
<Aside type="note">
  The `/fs/upload` endpoint is a streaming upload path. ConvexFS forwards the
  request body directly to object storage and does not buffer the full file in
  the Convex HTTP action first. If `Content-Length` is missing, ConvexFS counts
  bytes while streaming so the committed metadata still records the correct
  size.
</Aside>

<Aside type="tip">
  For high-volume Bunny uploads, configure `uploadMode: "bunny-edge-presigned"`
  and use `fs.createUploadUrl()` from a Convex action. The browser or CLI
  uploads bytes to the returned Bunny Edge Script URL, then calls a mutation
  that runs `fs.registerUploadedBlob()` and `fs.commitFiles()`. This keeps
  Convex on the control plane: auth, blob allocation, metadata, commit, and
  garbage collection.
</Aside>
