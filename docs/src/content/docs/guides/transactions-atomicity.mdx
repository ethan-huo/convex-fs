...
export const processAndDelete = action({
  ...
  handler: async (ctx, args) => {
    ...
    const data = await fs.getFileStream(ctx, args.path);
    if (!data) {
      throw new Error("File disappeared before streaming");
    }
    await sendToExternalService(data.stream);
...
```typescript
export const transcodeViaWorker = action({
  args: { path: v.string() },
  handler: async (ctx, args) => {
    ...
    const image = await fs.getFileStream(ctx, args.path);
    if (!image) {
      throw new Error("File disappeared before streaming");
    }

    // Send the current bytes to an external worker that returns a transformed stream
    const processed = await fetch("https://example.com/transcode", {
      method: "POST",
      headers: { "Content-Type": image.contentType },
      body: image.stream,
      duplex: "half",
    });
    if (!processed.ok || !processed.body) {
      throw new Error("Image processing failed");
    }

    // Upload the processed stream
    const { blobId } = await fs.writeBlobStream(
      ctx,
      processed.body,
      processed.headers.get("Content-Type") ?? "image/jpeg",
      {
        contentLength: processed.headers.get("Content-Length")
          ? Number(processed.headers.get("Content-Length"))
          : undefined,
      },
    );
    ...
    await fs.commitFiles(ctx, [
      { path: args.path, blobId, basis: file.blobId },
    ]);
  },
});
```
