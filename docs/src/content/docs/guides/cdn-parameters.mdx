...
2. When ConvexFS processes the download request, it decodes these parameters and
   appends them to the CDN URL
3. If token authentication is enabled, the parameters are included in the
...
included in the HMAC-SHA256 signature that Bunny.net validates. This means:
...
![Bunny.net Edge Rule for Content-Disposition](./cdn-params-filename.png)
