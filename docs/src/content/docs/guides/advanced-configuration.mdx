...
| `uploadMode`      | `"convex-proxy" \| "bunny-edge-presigned"` | No                                                     | Upload strategy. Defaults to `"convex-proxy"` for the `/fs/upload` route. Use `"bunny-edge-presigned"` with a standalone Bunny Edge Script signer to keep upload bytes off Convex.                                    |
| `edgeUpload`      | `object`                                   | Required only for `uploadMode: "bunny-edge-presigned"` | Signer configuration: `{ signUrl, accessKey?, headers? }`. The signer should return the signed upload URL as text.                                                                                                    |
...
const fs = new ConvexFS(components.fs, {
  storage: {
    ...
    edgeUpload: {
      signUrl: process.env.BUNNY_EDGE_SIGN_URL!,
      accessKey: process.env.BUNNY_EDGE_ACCESS_KEY,
    },
  },
});
```

Provision the standalone signer from setup scripts, CI, or admin tooling with
the non-runtime helper entrypoint:
...
Do not call this helper from Convex functions or application runtime code. It
uses the account-level Bunny API key and storage access key to create or update
the signer, bind the hostname, and configure signer-only secrets.
...
Authentication callback for uploads. Called before any upload is accepted, and
before any of the request body is read.

```typescript
uploadAuth: async (ctx: HttpActionCtx, info: UploadRequestInfo) =>
  Promise<boolean | UploadAuthDecision>;
```

Return `true`/`false` to allow or deny, or a decision object to allow with a
...
interface UploadRequestInfo {
  ...
  contentType: string;
  contentLength?: number; // client-supplied claim, not enforcement
}

interface UploadAuthDecision {
  allowed: boolean;
  maxBytes?: number; // enforced while streaming; 413 if exceeded
}
```

A denial rejects the request with a 403 response.

See [Authn & Authz](/guides/authn-authz#upload-authentication) for details,
including why `uploadAuth` is not the right place for path-scoped authorization.

### allowedHeaders (optional)

Extra request headers to permit on cross-origin requests, merged with the
built-in set (`Content-Type`, `Content-Length`, `Authorization`).

```typescript
allowedHeaders: ["X-Project-Id"];
```

Add any custom header your client sends, or the browser's CORS preflight will
reject the upload.
