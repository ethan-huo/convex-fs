...
First, install the component directly from GitHub. This fork is maintained as a
GitHub dependency, not as an npm-published package.
...
Then, add the component to your app by registering it in your `convex.config.ts`
file, like so:
...
<Aside type="caution">
  If you're upgrading an existing app from an older release, read [Migration:
  Streaming-Only Fork](/guides/migration-streaming-only) before you touch your
  data-transfer code. The old buffered client APIs were removed on purpose.
</Aside>
