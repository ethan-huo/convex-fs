...
- Testing -- the in-memory `test` storage backend now shares a single store
  process-wide, so a blob written by one Convex function is visible to the next.
  Previously every call site built its own empty store, which made round-trips
  through the test backend impossible. Because the store now outlives an
  individual `convexTest` instance, call `resetTestBlobStore()` -- newly
  exported from `convex-fs/test`, alongside `createTestBlobStore()` -- in
